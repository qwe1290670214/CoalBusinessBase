	...
	<script>
	   ...
	   $(document).ready(function(){
		   $(".dropDown_rows").click(function(){
	    		 ...
	    		 var j=$(this).children("span:eq(2)").html();
...
<body>
 	<input type="hidden" value="<%=basePath %>" id="path">
	        ...
	        <div class="panel-body container">
               <form class="form-inline" id="organAddForm" action="<%=basePath %>orgin/organControl?action=updateOrgan&organUUID=${entity.organUUID}" method="post">
				  <div class="form-group">
				    <label for="organName" class="labelField">组织机构名称：</label>
				    <input type="text" class="form-control"   value="${entity.organName}" name="organName" id="organName"placeholder="请输入名称" >
				  </div>
				 
				  <br>
				  <div class="form-group">
				    <label for="account" class="labelField">银行账户：</label>
				    <input type="text" class="form-control" value="${entity.accountCode}" name="account" id="account"placeholder="请输入组织机构账户" >
				  </div>
				  <div class="form-group">
				    <label for="organType" class="labelField">组织机构类型：</label>
				    <input type="text" class="form-control" value="${entity.organType}" name="organType" id="organType" placeholder="请输入类型">
				  </div>
				  <br>
				  <div class="form-group">
				    <label for="inuse" class="labelField">启用/禁用：</label>
				    <select class="form-control input-sm" name="inuse" id="inuse">
				              <c:if test="${entity.inUse=='启用'}">
					              <option value="1" selected>启用</option>
								   <option value="0"> 禁用</option>
				              </c:if>
				              <c:if test="${entity.inUse=='禁用'}">
				   ...
				   <input type="hidden" id="hideParentOrgan" value="${entity.parentUUID}" name="hideParentOrgan"> <!-- 存向后台传的上级组织机构编号 -->
				  <div class="form-group">
				        <label for="inuse" class="labelField">上级组织机构：</label>
				     <div class="btn-group">
						 	<button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						    <span id="dropdownOrgan">
						       <c:if test="${parentOrganName!=null}">${parentOrganName}</c:if> 
						       <c:if test="${parentOrganName==null}">选择类型</c:if> 
				  ...
				  <div class="form-group">
				    ...
				    <textarea class="form-control" id="desc" name="desc" rows="3" cols="40">${entity.description}</textarea>
				  </div> 
